...
기준으로 삼는 만세력 앱의 구현과 정합을 위해, 이 프로젝트는 **24절기(15°)** 경계를 월 판단에 사용합니다.

- 구현 방식
  - 태양 황경이 **15° 격자(0, 15, 30, ... 345)** 를 통과하는 시각(절기 경계)을 Skyfield로 계산
  - 특정 시각의 “가장 최근 절기 경계(15°)”를 찾은 다음
  - **15° 경계를 2개씩 묶어 12개월(寅월=1..丑월=12)** 로 매핑
    - 예: 백로(165°) 이후에는 앱에서 월지가 酉로 넘어가는 케이스가 확인됨

- 절기 경계 시각은 Skyfield로 계산한 태양 황경 통과 시각을 사용합니다.
- 경계가 없는 날(대부분의 날)은 “가장 최근 과거의 절기(15°) 경계”를 찾아 그 각도에 따라 절기월을 판정합니다.

### 시간 미상 정책 C(월주 후보 2개)

- 출생시간이 미상(`birth_time=null`)이고, 해당 KST 날짜(0:00~23:59)에 **절기(15°) 경계**가 포함되면
- 경계 이전/이후로 월주가 달라질 수 있어 **월주 후보 2개**를 반환합니다.
